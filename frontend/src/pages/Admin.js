...
import { listCompanies, createCompany, listJobs, createJob, matchJob, applyJob } from '../api_admin'
...
export default function Admin(){
  ...
  // read role synchronously but call hooks unconditionally
  const role = typeof window !== 'undefined' ? localStorage.getItem('nn_role') : null

  React.useEffect(()=>{
    // only attempt to load data if user is admin
  ...
  async function submitCompany(e){
    ...
    const res = await createCompany({name:f.name.value, description:f.description.value, website:f.website.value})
    if(res && res.id){
  ...
  async function submitJob(e){
    e.preventDefault()
    const f=e.target.elements
    const res = await createJob({title:f.title.value, description:f.description.value, required_skills:f.required_skills.value})
    if(res && res.id){
  ...
  async function doMatch(id){
    ...
    if(r && r.matches){
      setMsg(`Job Match Results for "${r.job}":\n${r.matches.map(m => `${m.register_number}: ${m.score}% match`).join('\n')}`)
    } else {
  ...
  return (
    <div>
      <div className="card">
        ...
        <ul>{companies.map(c=> <li key={c.id}><strong>{c.name}</strong> {c.website && <a href={c.website} target="_blank" rel="noopener noreferrer">🔗</a>}</li>)}</ul>
      </div>
      ...
      <div className="card">
        ...
        <h3>Posted Jobs</h3>
        <ul>{jobs.map(j=> <li key={j.id}>
          <strong>{j.title}</strong> @ {j.company?.name}
          <br/><small>Skills: {j.required_skills}</small>
